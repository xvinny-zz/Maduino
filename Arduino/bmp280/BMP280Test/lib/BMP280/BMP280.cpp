...
*/
char BMP280::begin(TwoWire *myWire, int sda, int scl)
{
    m_myWire = myWire;
...
char BMP280::readBytes(unsigned char *values, char length)
{
	...
	m_myWire->beginTransmission(BMP280_ADDR);
	m_myWire->write(values[0]);
	error = m_myWire->endTransmission();
	if (error == 0)
	{
		m_myWire->requestFrom(BMP280_ADDR,length);
		while(m_myWire->available() != length) ; // wait until bytes are ready
		for(x=0;x<length;x++)
		{
			values[x] = m_myWire->read();
		}
		return(1);
	}
	return(0);
